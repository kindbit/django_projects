...
class ManualProtect(View) :
    def get(self, request):
        if not request.user.is_authenticated :
            loginurl = reverse('admin:login')+'?'+urlencode({'next': request.path})
            return redirect(loginurl)
        return render(request, 'authz/main.html')

class ProtectView(LoginRequiredMixin, View) :
    login_url = 'admin/login'
    #redirect_field_name = 'authz/main.html'
    def get(self, request):
        return render(request, 'authz/main.html')
...
class DumpPython(View) :
    def get(self, req):
        resp = "<pre>\nUser Data in Python:\n\n"
        resp += "Login url: " + reverse('admin:login') + "\n"
        resp += "Logout url: " + reverse('admin:logout') + "\n\n"
